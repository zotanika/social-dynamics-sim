...
우리는 인간의 공감을 소규모의 느슨하게 연결된 집단(조상 환경)에서 분산과 스트레스를 흡수하기 위해 진화한 \emph{국소적 감쇠 메커니즘(local damping mechanism)}으로 해석한다.
그러나 지연 시간이 거의 없고 전역적인 도달 범위를 가진 현대의 초연결 네트워크---특히 디지털 환경---에서, 이와 동일한 메커니즘은 ``진화적 불일치(evolutionary mismatch)'' 영역에서 작동할 수 있다.

이러한 구조적 불일치를 배경으로, 우리는 두 가지 이상적인 정책/문화 체제를 비교한다:
(1) 개인의 자율성과 문제 해결을 장려하고, 적절한 스트레스를 학습과 역량 확장의 발판으로 삼는 \emph{적응형/자율 중심 체제}(사회 A);
(2) 공감, 보호, 외부 지원을 강조하며, 시스템 수준에서 개인의 스트레스를 빠르게 완충하려는 \emph{지원 중심 체제}(사회 B).
두 사회는 동일한 공감 분포와 동일한 초기 심리적 회복탄력성 분포를 공유한다.

우리는 인지된 스트레스를 공감($E$), 네트워크 결합($G$), 정책 체제의 함수로 정의하고, 이러한 요인들이 어떻게 상호작용하여 회복탄력성과 집단화(정체성 클러스터링)의 역학을 형성하는지 간단한 차분 방정식을 사용하여 모델링한다.
...
사회 A는 높은 회복탄력성 균형으로 수렴하는 경향이 있는 반면, 사회 B는 평균 회복탄력성 하락과 집단화 증가를 나타내는 경향이 있다.

이러한 구체적인 이분법적 모델 외에도, 우리는 스트레스가 공감과 집단화의 단조 함수로 생성되고, 회복탄력성이 현재 회복탄력성과 스트레스의 단조 함수로 갱신되는 보다 추상적인 프레임워크를 구축한다.
약한 구조적 가정 하에, 우리는 일반적인 단조성 결과를 증명한다: 어떠한 정책 체제, 공감 분포, 사회적 자원의 사용에 대해서도, 모든 시점에서 점별로 더 낮은 집단화 궤적은 모든 시점에서 약하게 더 높은 회복탄력성과 약하게 더 늦은 붕괴 시간을 산출한다.
이 장난감 모델 설정 내에서, 우리는 평균 회복탄력성이 임계값 아래로 떨어지는 ``마지노 시간($t_{\text{mag}}$)''을 정의하고, 단조성 가정이 유지되는 한 구조적으로 집단 해체를 장려하는 정책은 이 시점을 지연시킬 뿐(결코 가속화하지 않음)임을 보여준다.

이 모델은 의도적으로 단순화되었으며 실제 국가나 이데올로기를 문자 그대로 묘사하기 위한 것이 아니다. 오히려 공감, 네트워크 이득, 집단화, 정책 체제가 어떻게 상호작용하여 장기적인 사회적 취약성을 형성하는지에 대한 구조적 관점을 제시하는 것을 목표로 한다.
\end{abstract}
...
약한 구조적 가정 하에, 모든 시점에서 집단화나 정체성 클러스터링을 낮추는 모든 정책 경로는 체제의 세부 사항이나 공감 분포에 관계없이 회복탄력성을 개선하고 붕괴를 지연시킬 뿐이다.
...
우리는 공감을 단순한 미덕을 넘어 사회 시스템의 \textbf{피드백 이득 요소(feedback gain element)}로 해석한다.
...
\begin{itemize}
  ...
  \item 사회 A와 사회 B는 시간이 지남에 따라 회복탄력성 $R$과 집단화 $G$가 갱신되는 방식에서만 차이가 나며, 이는 서로 다른 정책/문화적 체제를 반영한다.
\end{itemize}
...
이 섹션에서는 한 걸음 물러나, 체계적으로 ``탈집단화''를 진행하는 것이 회복탄력성을 개선하고 붕괴를 지연시킨다는 핵심적인 질적 주장이 훨씬 더 일반적인 설정에서도 도출될 수 있음을 보여준다.
...
H(E,G_1) \le H(E,G_2).
\]
즉, 공감을 고정했을 때, 더 높은 집단화/결합은 인지된 스트레스를 결코 감소시키지 않는다.
\end{assumption}
...
비록 시뮬레이션이 특정 역학(예: 사회 A의 탈집단화 및 사회 B의 스트레스 유도 집단화)에 초점을 맞추고 있지만, 단조성 명제는 ``더 적은 집단화''가 광범위한 체제와 매개변수화 하에서 장기적 회복탄력성을 위한 구조적으로 안전한 방향임을 보여준다.
...
국소적 클러스터링이 평균장 예측보다 빠르게 스트레스를 증폭시키는 ``공명 구역(pockets of resonance, 반향실)''을 생성할 수 있지만, 전체적인 거시 역학은 동일하게 유지된다: 집단화 기반의 스트레스 축적을 촉진하는 정책은 결국 임계점(마지노 시간)에 도달한다.
