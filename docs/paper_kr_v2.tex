...
\begin{abstract}
본 논문은 정보 이론, 제어 이론, 그리고 정책 체제의 정형화된 모델을 결합한 사회적 스트레스 역학을 위한 이론적 프레임워크를 제안한다.
우리는 인간의 공감을 소규모의 약하게 결합된 집단(조상 환경)에서 분산과 스트레스를 흡수하기 위해 진화한 \emph{국소적 감쇠 메커니즘(local damping mechanism)}으로 취급한다.
그러나 지연 시간이 거의 없고 전역적인 도달 범위를 가진 현대의 초연결 네트워크---특히 디지털 환경---에서, 이와 동일한 메커니즘은 ``진화적 불일치(evolutionary mismatch)'' 영역에서 작동할 수 있다.

이러한 구조적 불일치 위에서, 우리는 두 가지 이상적인 정책/문화 체제를 비교한다:
(1) 개인의 자율성과 문제 해결을 장려하고, 적절한 스트레스를 학습과 능력 확장의 기회로 취급하는 \emph{적응형/자율 중심 체제}(사회 A);
(2) 공감, 보호, 외부 지원을 강조하며, 시스템 수준에서 개인의 스트레스를 빠르게 완충하려는 \emph{지원 중심 체제}(사회 B).
두 사회는 동일한 공감 분포와 동일한 초기 심리적 회복탄력성 분포를 공유한다.

우리는 인지된 스트레스를 공감($E$), 네트워크 결합($G$), 정책 체제의 함수로 정의하고, 이러한 요인들이 어떻게 상호작용하여 회복탄력성과 그룹화(정체성 클러스터링)의 역학을 형성하는지 간단한 차분 방정식을 사용하여 모델링한다.
국소적 네트워크 위상과 '좁은 세상(Small-World)' 상호작용을 포함하도록 확장된 최소한의 에이전트 기반 시뮬레이션을 통해, 우리는 동일한 초기 조건과 외부 자극 하에서도 사회 A와 사회 B가 시간이 지남에 따라 극적으로 갈라질 수 있음을 보여준다.
사회 A는 높은 회복탄력성 균형으로 수렴하는 경향이 있는 반면, 사회 B는 평균 회복탄력성 하락과 그룹화 증가를 나타내는 경향이 있다.

이러한 구체적인 이분법적 모델 외에도, 우리는 스트레스가 공감과 그룹화의 단조 함수로 생성되고, 회복탄력성이 현재 회복탄력성과 스트레스의 단조 함수로 갱신되는 보다 추상적인 프레임워크를 구축한다.
약한 구조적 가정 하에, 우리는 일반적인 단조성 결과를 증명한다: 어떠한 정책 체제, 공감 분포, 사회적 자원의 사용에 대해서도, 모든 시점에서 점별로 더 낮은 그룹화 궤적은 모든 시점에서 약하게 더 높은 회복탄력성과 약하게 더 늦은 붕괴 시간을 산출한다.
...
현대 사회는 여러 형태의 취약성을 보여준다: 극심한 양극화, 만성적인 스트레스, 그리고 사소해 보이는 갈등의 급격한 확대가 그것이다.
공론장과 사회 과학에서는 종종 이러한 문제를 도덕적 또는 이데올로기적 용어로 틀지곤 한다:
누가 옳고 그른가?  
공감이 너무 적은가, 아니면 너무 많은가?  
특정 반응이 ``옳은가'' 아니면 ``그른가''?

사회 과학의 실증 연구는 설문 설계, 통계 분석, 그리고 빅데이터 상관관계 분석 방법에서 점점 더 정교해지고 있다.
하지만 스트레스와 회복탄력성이 시간이 지남에 따라 어떻게 공진화하는지---특히 서로 다른 정책 체제와 네트워크 구조 하에서---에 대한 명시적인 \emph{역동적(dynamic)} 모델은 여전히 드물다.

복잡계와 진화 생물학의 관점에서 볼 때, 문제의 일부는 도덕적 범주보다는 \textbf{구조와 역학}에 있을 수 있다.
우리는 이를 \textbf{진화적 불일치(evolutionary mismatch)}~\cite{lloyd2011evolutionary}의 특수한 사례로 해석한다.
공감과 스트레스 반응 시스템을 포함한 인간의 심리적 기제는 네트워크 결합이 약하고 정보 지연이 상당했던 소규모 집단에서 형성되었다.
그러한 환경에서 공감은 국소적인 충격 흡수 장치로 작동한다: 개인의 스트레스를 집단 전체로 재분배함으로써 개인이 붕괴되는 것을 방지한다.

그러나 디지털 기술과 소셜 미디어는 사회적 상호작용의 위상을 바꾸어 놓았다.
물리적 지연은 거의 제로에 가깝게 줄어들었고, 연결성과 상호작용 빈도는 폭발적으로 증가했다.
제어 이론의 용어로 설명하자면, 이는 사회적 신호 처리 시스템의 \emph{루프 이득(loop gain)}을 극적으로 증가시킨다.
낮은 이득에서 안정성을 유지하도록 설계된 피드백 루프가 높은 이득에서는 시스템을 진동시키거나 발산하게 만들 수 있다는 점은 공학적으로 잘 알려진 사실이다.

이러한 구조적 변화 위에, 사회는 스트레스를 어떻게 \emph{해석하고 관리}할지에 대해 매우 다른 규범과 정책을 채택한다.
일부 체제(우리의 사회 A)는 자율성, 자기 조절, 그리고 도전에 맞서는 것을 강조한다.
다른 체제(우리의 사회 B)는 보호, 공감, 그리고 외부 지원을 강조한다.
두 방향 모두 윤리적인 동기를 가질 수 있지만, 고이득 네트워크와 결합될 때 매우 다른 장기적인 역학을 초래할 수 있다.

본 논문에서는 Shannon의 채널 용량~\cite{shannon1948}, 시스템 역학~\cite{forrester1961,forrester1971}, 그리고 Pentland의 사회 물리학~\cite{pentland2014}에서 영감을 받아 공감, 네트워크 이득, 정책 체제를 사회적 스트레스 및 회복탄력성과 연결하는 최소한의 동역학 모델을 구축한다.
먼저 구체적인 이분법적 모델(사회 A 대 사회 B)을 분석한 후, 이를 일반적인 단조성 결과를 증명할 수 있는 보다 추상적인 프레임워크에 어떻게 적용할 수 있는지 보여준다:
약한 구조적 가정 하에, 모든 시점에서 그룹화나 정체성 클러스터링을 낮추는 모든 정책 경로는 체제의 세부 사항이나 공감 분포에 관계없이 회복탄력성을 개선하고 붕괴를 지연시킬 뿐이다.
우리의 목표는 데이터를 맞추는 것이 아니라, 초연결 사회에서의 구조적 취약성에 대해 생각할 수 있는 간결하고 형식적인 렌즈를 제공하는 것이다.
...
높은 공감은 타인의 감정 상태와 스트레스 신호에 대한 높은 민감도를 의미한다.

\begin{itemize}
  \item \textbf{약하게 결합된 네트워크 (국소적 감쇠).}  
  연결이 드물고 지연 시간이 길 때, 개인의 스트레스는 소수의 이웃과 공유된 후 사라진다.
  이러한 설정에서 공감은 주로 스트레스를 국소적으로 재분배하며 음의 피드백(negative feedback) 메커니즘으로 작동한다.

  \item \textbf{강하게 결합된 네트워크 (전역적 공명).}  
  연결이 조밀하고 지연 시간이 짧을 때, 스트레스 신호는 넓고 빠르게 퍼진다.
  공감 수준이 높은 개인들은 이러한 신호를 흡수하여 이차적인 스트레스를 경험하고 이를 다시 재전송한다.
  루프 이득이 충분히 크면, 공감은 스트레스를 감쇠시키기보다 증폭시키는 양의 피드백(positive feedback) 루프의 일부가 된다.
\end{itemize}
...
이 섹션에서는 스트레스와 회복탄력성 역학을 위한 구체적인 이분법적 모델을 명시한다.
섹션~\ref{sec:general}에서는 한 걸음 물러나 이 모델을 일반적인 구조적 결과를 도출할 수 있는 보다 추상적인 프레임워크에 삽입할 것이다.
...
\begin{itemize}
  ...
  \item $G_t \in [0,\infty)$: 시간 $t$에서의 그룹화 / 네트워크 결합 수준의 스칼라 지표,
  \item $\alpha > 1$: 비선형 지수; $\alpha$가 높을수록 공감 능력이 높은 개인이 불균형하게 더 큰 스트레스를 경험함,
  \item $\beta \ge 0$: \textbf{네트워크 이득} 매개변수; $\beta$가 높을수록 동일한 그룹화 수준 $G_t$가 더 많은 증폭을 생성함.
\end{itemize}
...
\begin{itemize}
  ...
  \item 사회 A와 사회 B는 시간이 지남에 따라 회복탄력성 $R$과 그룹화 $G$가 갱신되는 방식에서만 차이가 나며, 이는 서로 다른 정책/문화적 체제를 반영한다.
\end{itemize}
...
두 사회 모두에서 스트레스는 각각의 그룹화 지표를 사용하여~\eqref{eq:stress}로부터 계산된다:
\[
...
\begin{itemize}
  ...
  \item $\eta_A > 0$: 사회 A가 집단 라벨의 중요성을 약화시키는 속도(``탈그룹화''),
  \item $\eta_B > 0$: 사회 B에서 그룹화가 자연적으로 쇠퇴하는 속도(일반적으로 더 작음),
  \item $\gamma_B > 0$: 사회 B에서 스트레스가 그룹화 및 정체성 기반 동원을 유도하는 민감도.
\end{itemize}
...
\begin{itemize}
  ...
  \item $\beta \ge 0$: 네트워크 이득; 그룹화 $G$가 스트레스를 얼마나 강하게 증폭하는지 제어;
  ...
  \item $\eta_A > 0$: 사회 A의 탈그룹화 비율;
  \item $\eta_B > 0$: 사회 B의 탈그룹화 비율;
  \item $\gamma_B > 0$: 사회 B의 전체 스트레스에 대한 그룹화 민감도;
  \item $R_{\text{crit}}$: 마지노 시간을 정의하는 데 사용되는 임계 회복탄력성 문턱값.
\end{itemize}

본 논문에서 수치 데이터는 개념적 설명을 위해 선택되었으며, 실증적 캘리브레이션은 시도하지 않는다.

\section{그룹화에 대한 일반적인 구조적 결과}
\label{sec:general}

위의 이분법적 모델은 스트레스와 회복탄력성이 어떻게 진화하는지에 대해 강하고 구체적인 가정을 한다.
이 섹션에서는 한 걸음 물러나, 체계적으로 ``탈그룹화''를 진행하는 것이 회복탄력성을 개선하고 붕괴를 지연시킨다는 핵심적인 질적 주장이 훨씬 더 일반적인 설정에서도 도출될 수 있음을 보여준다.
...
다음과 같이 가정한다:
\begin{itemize}
  \item 공감 $E$와 그룹화 수준 $G$를 음이 아닌 스트레스로 매핑하는 \emph{스트레스 함수} $H(E,G)$:
  \[
  ...
  \]
  \item 초기 조건 $R_i(0)$가 주어지며 우리가 비교하는 시나리오들에 걸쳐 동일하다.
\end{itemize}

이제 $H$와 $F$에 대해 약한 단조성 가정을 부과한다.

\begin{assumption}[그룹화는 스트레스를 증가시킨다]
\label{ass:group-stress}
모든 $E$와 모든 $G_1 \le G_2$에 대해,
\[
H(E,G_1) \le H(E,G_2).
\]
즉, 공감을 고정했을 때, 더 높은 그룹화/결합은 인지된 스트레스를 결코 감소시키지 않는다.
\end{assumption}
...
단지 (i) 그룹화가 스트레스를 증폭시키고, (ii) 스트레스가 회복탄력성을 마모시키며, (iii) 회복탄력성이 음이 아닌 이월 효과(carryover effect)를 가짐을 명시할 뿐이다.

그룹화 경로 $\{G_t\}_{t\ge 0}$와 초기 조건 $\{R_i(0)\}$이 주어지면, 이러한 가정들은 다음을 통해 회복탄력성 궤적 $\{R_i^{(G)}(t)\}_{t\ge 0}$을 결정한다:
\[
...
\subsection{그룹화에서의 단조성}

동일한 초기 조건과 체제 매개변수를 공유하지만 시간이 지남에 따라 그룹화 수준이 다른 두 개의 그룹화 경로 $G_t$와 $\tilde{G}_t$를 비교한다.

\begin{proposition}[낮은 그룹화는 회복탄력성을 개선한다]
\label{prop:grouping}
가정~\ref{ass:group-stress}--\ref{ass:res-monotone}이 유지된다고 가정하자.
$\{G_t\}_{t\ge 0}$와 $\{\tilde{G}_t\}_{t\ge 0}$가 다음과 같은 두 그룹화 경로라고 하자:
\[
...
직관적으로, 낮은 그룹화는 스트레스를 줄이고(가정~\ref{ass:group-stress}), 낮은 스트레스는 회복탄력성을 해치지 않으며(가정~\ref{ass:stress-resilience}), 높은 회복탄력성은 미래의 회복탄력성을 해치지 않는다(가정~\ref{ass:res-monotone}).
이러한 단조성들은 시간의 흐름에 따라 그리고 인구 전체로 전파된다.

\subsection{마지노 시간에 대한 시사점}

어떠한 그룹화 경로 $G$에 대해서도 일반화된 마지노 시간을 다음과 같이 정의할 수 있다:
\[
t_{\text{mag}}(G) = \min\{ t \,:\, \bar{R}^{(G)}(t) \le R_{\text{crit}}\},
\]
임계값을 결코 넘지 않는 경우 $t_{\text{mag}}(G) = +\infty$로 본다.

\begin{proposition}[낮은 그룹화는 붕괴를 지연시킨다]
\label{prop:maginot}
...
명제~\ref{prop:maginot}는 단조성 가정이 유지되는 한, 구조적으로 그룹 해체를 장려하는(즉, 각 시점에서 그룹화를 낮게 유지하는) 어떠한 정책 궤적도 회복탄력성을 악화시키거나 붕괴를 앞당길 수 없다는 직관을 공식화한다.
...
\begin{itemize}
  \item 다른 경로 $G_t$보다 점별로 더 낮은 그룹화 경로 $\tilde{G}_t$를 산출하는 어떠한 정책 조합도 모든 시점에서 약하게 더 높은 회복탄력성을 생성할 것이다.
  \item $\tilde{G}_t$ 하에서의 마지노 시간은 $G_t$ 하에서보다 약하게 더 늦을 것이다.
\end{itemize}

이 일반적인 결과는 구체적인 A/B 시뮬레이션을 보완한다:
비록 시뮬레이션이 특정 역학(예: 사회 A의 탈그룹화 및 사회 B의 스트레스 유도 그룹화)에 초점을 맞추고 있지만, 단조성 명제는 ``더 적은 그룹화''가 광범위한 체제와 매개변수화 하에서 장기적 회복탄력성을 위한 구조적으로 안전한 방향임을 보여준다.
...
\begin{itemize}
  ...
  \item 초기 회복탄력성 $R_i(0)$와 초기 그룹화 $G_0$ 또한 사회 간에 동일하다.
  \item 인지된 스트레스 $S_i(t)$는~\eqref{eq:stress}를 통해 계산되며, $G_t^A$와 $G_t^B$는~\eqref{eq:GA}--\eqref{eq:GB}에 따라 진화한다.
...
\begin{figure}[t]
  ...
  사회 A는 점진적으로 그룹화를 줄이고 적절한 스트레스를 회복탄력성 성장으로 전환하여 높은 회복탄력성 균형으로 수렴한다.
...
\begin{itemize}
  \item 사회 A에서는 $G_t^A$가 $\eta_A$의 비율로 감소하고, $k_A R(1-R)$이 성장을 촉진하며 마모 항은 완만하다.
    시스템은 낮은 그룹화와 함께 높은 평균 회복탄력성에서 안정화된다.
...
\begin{itemize}
    ...
    \item 그룹화 $G$는 국소 변수가 된다; 에이전트 $i$의 경우, 전역 평균이 아니라 이웃들의 국소적 불안에 의해 스트레스가 영향을 받는다.
    \item 구체적으로, 사회 B의 스트레스 주도형 그룹화 갱신은 인구 평균 대신 $C_{i}^B = \text{mean}(S_{\text{이웃 of } i})$를 사용한다.
\end{itemize}

시뮬레이션 결과는 사회 A와 사회 B 사이의 질적 발산이 이러한 위상학적 변화에도 강건함을 확인시켜준다.
국소적 클러스터링이 평균장 예측보다 빠르게 스트레스를 증폭시키는 ``공명 주머니(pockets of resonance, 반향실)''를 생성할 수 있지만, 전체적인 거시 역학은 동일하게 유지된다: 그룹화 기반의 스트레스 축적을 촉진하는 정책은 결국 임계점(마지노 시간)에 도달한다.
...
그림~\ref{fig:gain}은 이득 매개변수 $\beta$의 결정적인 역할을 보여준다.
$\beta$가 낮을 때 네트워크는 수동적인 매개체로 작용하며 지원 중심 체제는 실행 가능하다.
그러나 $\beta$가 임계 문턱값을 넘으면 스트레스와 그룹화 사이의 양의 피드백 루프가 지배하게 되어 지원 메커니즘을 불충분하게 만든다.
이는 ``규모의 역설''이 근본적으로 네트워크의 \emph{이득(gain)}에 의해 주도됨을 시사하며, 현대 디지털 기술은 조상 환경에 비해 이 이득을 수십 배 증가시켰다.
...
$t_{\text{mag}}^B$ 이후에는 평균 회복탄력성이 너무 낮아져 작은 충격조차 많은 개인들을 $R_i^B \approx 0$ 근처로 밀어낼 수 있다.
이 영역에서는 단순히 $\beta$를 줄이는 사후 개입(예: 플랫폼의 강력한 규제나 폐쇄)만으로는 대규모의 지속적인 외부 조치 없이는 회복탄력성을 복구하는 데 어려움을 겪을 수 있다.
...
공적 토론은 종종 사회적 스트레스의 \emph{콘텐츠}(예: 유해한 언사, 가짜 뉴스)나 공감의 \emph{도덕적 자질}(너무 적음 대 너무 많음)에 집중한다.
이러한 관점들은 중요하지만 불완전할 수 있다.

우리의 모델은 상대적으로 온건한 콘텐츠라 할지라도 고이득 네트워크에 의해 반복적으로 증폭될 때 상당한 스트레스를 유발할 수 있으며, 장기적인 효과는 정책 체제와 그룹화 역학에 크게 의존함을 시사한다:
\begin{itemize}
...
따라서 문제는 ``공감 대 무공감'' 또는 ``좋은 콘텐츠 대 나쁜 콘텐츠'' 단독의 문제가 아니라, 서로 다른 정책 선택 하에서 공감, 콘텐츠, 네트워크 이득, 그룹화가 어떻게 상호작용하느냐의 문제이다.
...
일반적인 구조적 결과는 광범위한 모델 클래스 내에서 하나의 설계 선택이 특히 강력한 효과를 가짐을 보여준다:
그룹화를 쇠퇴시키거나 강력한 정체성 클러스터 형성을 피하는 정책은 공감이 어떻게 분포되어 있든 사회적 자원이 어떻게 배분되어 있든 회복탄력성에 약하게 유익하다.
공감적 지원과 보호 정책은 본질적으로 문제가 아니다; 그들의 장기적인 영향은 그것들이 그룹화와 네트워크 이득을 증가시키거나 감소시키는 메커니즘과 결합되어 있는지 여부에 결정적으로 달려 있다.
...
우리는 공감, 네트워크 이득, 그룹화, 정책 체제를 사회적 스트레스와 회복탄력성의 진화에 연결하는 간단한 동역학 프레임워크를 제안했다.
조상의 저이득 환경에서 공감은 주로 국소적 감쇠 메커니즘으로 작동했을 가능성이 크다.
현대의 고이득 네트워크에서 동일한 공감 분포는 체제가 더 적응형/자율 중심적인지(사회 A) 아니면 더 지원 중심적인지(사회 B)에 따라 매우 다르게 작동할 수 있다.

이 장난감 모델 내에서 마지노 시간이 나타난다: 특정 구성 하에서 시스템이 회복을 위한 구조적 능력을 상당 부분 상실하게 되는 유한한 지평이다.
좀 더 추상적인 수준에서, 우리는 약한 단조성 가정 하에 모든 시점에서 그룹화를 낮게 유지하는 어떠한 정책 궤적도 체제의 세부 사항이나 공감 분포에 관계없이 회복탄력성을 약하게 개선하고 붕괴를 지연시킴을 보여주었다.

목표는 공감이나 정의에 대한 규범적 논쟁을 해결하는 것이 아니라, 그러한 논쟁이 피드백, 이득, 그룹화, 그리고 역학에 대한 명시적인 고려로 보완되어야 함을 제안하는 것이다.
우리는 이 프레임워크가 콘텐츠나 도덕성과 더불어 시스템의 구조적 및 역학적 속성이 함께 고려되는 사회 정책, 정신 건강, 그리고 플랫폼 설계에 관한 향후 토론을 위한 개념적 도구로 활용되기를 희망한다.

\appendix

\section{Python 시뮬레이션 코드 (확장판)}
\label{app:code}
...
\begin{lstlisting}[language=Python, caption=Social Dynamics Simulation Code (v2)]
import numpy as np
