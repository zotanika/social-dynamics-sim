...
우리는 인간의 공감을, 소규모의 느슨하게 연결된 집단(조상 환경)에서 분산과 스트레스를 흡수하기 위해 진화한 \textbf{'국소적 감쇠 기제(local damping mechanism)'}로 이해한다.
...
우리는 인지된 스트레스를 공감($E$), 네트워크 결합($G$), 그리고 정책 체제의 함수로 정의한다. 이 요인들이 상호작용하여 어떻게 회복탄력성과 \textbf{집단화(정체성 클러스터링)}의 역학을 형성하는지, 간단한 차분 방정식을 통해 모델링한다.
...
공감을 포함한 인간의 심리 기제는 네트워크 연결이 느슨하고 정보 전달이 느린 소규모 집단에서 형성되었다.
